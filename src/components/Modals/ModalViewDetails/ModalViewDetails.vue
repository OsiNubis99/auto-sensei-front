<template>
    <div v-show="statusModal.isActive"
        ...
        statusModal.dataAutiont?.vehicleDetails?.model }}</p>
                    ...
                    <div v-if="statusModal.dataAutiont?.status == 'bids completed'" class="hidden md:flex gap-2 items-center">
                    ...
                    <div v-if="statusModal.dataAutiont?.status == 'bids completed'" class=" md:hidden p-2 fixed bottom-0 w-full flex bg-white gap-2 items-center">
                        <button class="btn w-full bg-primary rounded-lg">Accept</button>
                        <button class="btn w-full bg-white border border-error text-error rounded-lg">Decline</button>
                    </div>
                </div>
                        ...
                        <div>
                            <div class="flex justify-between gap-2 md:hidden">
                                ...
                                <div class="w-full flex flex-col gap-2">
                                    ...
                                    <p class="text-xs">Approval ends in <span class="text-[#FF9A02]"> 48 Hours </span> </p>
                                </div>
                            </div>
                            <img v-if="statusModal.dataAutiont?.photos"
                                class="md:h-[400px] w-[30%] h-[80px] shadow-xl hidden md:block md:w-full rounded-lg object-cover  "
                                :src="bucket + statusModal.dataAutiont?.photos[0]?.url" alt="">
                            <img  v-else class="md:h-[400px] w-[30%] h-[80px] shadow-xl hidden md:block md:w-full rounded-lg object-cover "
                                src="@/assets/img/jpg/image.jpg" alt="">
                            <div class="flex flex-col gap-1">
                                <div class="grid grid-cols-2 gap-2 pt-4">
                                    <div v-if="statusModal.dataAutiont?.vin" class="flex gap-2 items-center">
                                        ...
                                        <p class="text-[10px] md:text-base  uppercase ">{{ statusModal.dataAutiont?.vin }}</p>
                                    </div>
                                    <div class="flex gap-2 items-center">
                                        ...
                                        <p class="text-[10px] md:text-base  capitalize ">{{ statusModal.dataAutiont?.vehicleDetails.odometer }}
                                            Kilometers</p>
                                    </div>
                                </div>
                                <div class="grid grid-cols-2 gap-2 pb-4">
                                    <div class="flex gap-2 items-center">
                                        ...
                                        <p class="text-[10px] md:text-base capitalize">{{ statusModal.dataAutiont?.vehicleDetails.color }}</p>
                                    </div>
                                    <div class="flex gap-2 items-center">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16"
                                            fill="none">
                                        ...
                                        <p class="text-[10px] md:text-base capitalize">{{ statusModal.dataAutiont?.vehicleDetails.driveTrain }}</p>
                                    </div>
                                </div>
                            </div>
                            <div class="flex gap-2 items-center mb-4">
                                <div class="bg-[#F0F0F0] flex px-2 md:px-5 py-1 gap-1 md:gap-3 rounded-lg items-center">
                                    ...
                                    <p class=" text-[10px] md:text-base capitalize">{{ statusModal.dataAutiont?.vehicleDetails.tireCondition }}</p>
                                </div>
                                <div class="bg-[#F0F0F0] flex px-2 md:px-5 py-1 gap-1 md:gap-3 rounded-lg items-center">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 14 14"
                                        fill="none">
                                    ...
                                    <p class=" text-[10px] md:text-base capitalize">{{ statusModal.dataAutiont?.vehicleDetails.brakeCondition }}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class=" h-[50vh] md:p-4 overflow-x-hidden overflow-y-auto ">
                            <template v-if="statusModal.dataAutiont?.bids.length > 0"
                                ...
                                <div
                                            ...
                                            <div class="flex gap-1 p-2 flex-col items-start">
                                                <template
                                                    ...
                                                    <div class="flex gap-2 items-center">
                                                        ...
                                                        <p class="md:px-4 md:py-1 py-1 px-2 rounded-full text-[10px] md:text-base  bg-[#05A54B] text-white">Winner</p>
                                                    </div>

                                                    <p class="text-[10px] md:text-lg whitespace-pre">Oct 20, 2023 at 2:41 AM</p>
                                                </template>
                                                <template v-else>
                                                    <p class=" truncate w-[150px] md:w-fit font-semibold">{{ dealerBit.participant.dealer.name }}</p>
                                                    <p class="text-[10px] md:text-lg whitespace-pre">{{ moment(dealerBit?.participant?.updatedAt).format('MMMM DD YYYY, h: mm: ss a') }}</p>
                                                </template>
                                    ...
                                    :to="{ name: 'inbox-seller', query: { id: statusModal.dataAutiont?._id + '-' + statusModal.dataAutiont?.bids[0].participant._id }}"
                                        class="w-full btn border hover:bg-primary transition-all ease-in duration-300  hover:text-white py-2 rounded-lg md:mt-4 border-[#F0F0F0] ">Contact
                                        Buyer</RouterLink>
                                </div>
