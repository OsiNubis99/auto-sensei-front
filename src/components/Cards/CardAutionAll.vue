    ...
    <div class="w-full flex   sm:p-0 relative" :class="changeLayouts ? 'flex-col' : 'flex-col md:flex-row'">
        ...
        <div class="w-full flex  justify-between gap-3 " :class="changeLayouts ? 'flex-col' : 'flex-col md:flex-row'">
            <div @click="(auction?.status == 'live' || auction?.status == 'bids completed' || auction?.status == 'completed') && statusModalView.openModal({ isActive: true, data: auction })"
                :class="auction?.status == 'live' || auction?.status == 'bids completed' || auction?.status == 'completed' ? 'cursor-pointer ' : ''"
        ...
        auction?.vehicleDetails?.brakeCondition }}</p>
                ...
                <RouterLink v-if="auction?.status == 'bids completed'" to="#"
                    ...
                    <div class="bg-[#f0f0f080] hidden md:flex gap-3 py-1 px-2  rounded-md items-center">
                        ...
                        <p class=" text-md font-semibold capitalize ">Contact Buyer</p>
                    </div>
                </RouterLink>
