<template>
    <div class=" fixed z-[100] bottom-2 w-full gap-3 px-4 flex justify-between items-center ">
        <RouterLink v-if="path !== 'draft'" to="/draft"
            class=" btn w-full !py-2 !text-sm bg-white border border-base-black font-medium">
            Draft
            <span class="text-[#858585] ml-1">{{ autions?.draft?.length }}</span>
        </RouterLink>
        <RouterLink to="/all/create" id="createAuction"
            class="btn w-full !py-2 text-primary flex justify-between gap-2 !text-sm bg-base-black border font-medium ">
            ...
            <p class="text-sm mt-1 whitespace-nowrap ">Create Auction</p>
        </RouterLink>
    </div>
</template>
