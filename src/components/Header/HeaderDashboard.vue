<template>
  <header class="relative">
      ...
      <nav class="lg:flex bg-base-black relative shadow-lg px-3 py-2 justify-between flex-row-reverse">
        ...
        <div class="flex items-center gap-3">
          <div class="flex w-full md:w-fit justify-between md:justify-normal items-center gap-2">
            ...
            <button v-if="!opeNav" @click="openNavSlider" class="flex md:hidden items-center px-3 py-2 text-white rounded text-teal-lighter hover:text-white hover:border-white">
            ...
            <button v-else @click="closetNavSlider">
              <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-8 md:w-8 md:h-8 cursor-pointer" fill="none" viewBox="0 0 24 24" stroke="#fff">
          ...
          <div
            :class="opeNav ? 'bg-base-black left-0 absolute h-screen z-[100] ' : 'hidden md:block '"
          ...
          >
            <ul class="flex md:flex-row flex-col md:items-center md:gap-[2vw] gap-8">
              ...
              <RouterLink
                :to="{ name: 'action-list', query: { state: 'drafts' } }"
                @click="closetNavSlider"
      ...
      <nav class="flex bg-base-black relative shadow-lg px-3 py-2 justify-between flex-row-reverse">
        <div class="hidden lg:flex items-center">
          <div class="flex gap-8 justify-between items-center">
            <div class="relative">
              <svg class="w-6 h-6 text-gray-700 absolute top-3 left-2" fill="none" stroke="white" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path>
              </svg>
              <input type="search" placeholder="Search for vehicles by make, model, VIN, etc." class="border-[1px] rounded-[8px] text-white w-full bg-transparent border-[#d3d3d361] py-3 pl-10 pr-3" />
            </div>
            ...
            <div class="navbar-right relative">
              <button class="relative z-50 block overflow-hidden leading-loose rounded cursor-pointer" @click="isOpen = !isOpen">
                <div class="w-8 h-8">
                  <img v-if="store?.userData?.seller?.picture" class="w-full shadow-md rounded-full h-full object-cover" :src="bucket + store?.userData?.seller?.picture" alt="" />
                  <img
                    v-else
              ...
              <div v-if="isOpen" class="absolute top-auto right-[-100%] w-48 py-2 mt-2 mr-10 rounded-lg border-gray-900 bg-white shadow-xl z-[9999]">
                <div @click="goAccount" class="text-sm text-gray-900 hover:bg-base-black hover:text-primary block px-4 py-2 cursor-pointer">Account Settings</div>
                <div @click="logout" class="text-sm text-gray-900 hover:bg-base-black hover:text-primary block px-4 py-2 cursor-pointer">Sign Out</div>
              </div>
            </div>
          </div>
          <div name="menu" @click="toggle()" class="text-[30px] cursor-pointer md:hidden">menu</div>
        ...
        <div class="block lg:hidden">
          <div class="flex justify-center gap-2 items-center">
            ...
            <button @click="toggle" class="flex items-center px-3 py-2 text-white rounded text-teal-lighter hover:text-white hover:border-white">
              <svg class="fill-current h-4 w-4" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                <title>Menu</title>
                <path d="M0 3h20v2H0V3zm0 6h20v2H0V9zm0 6h20v2H0v-2z" />
              </svg>
        ...
        <div class="flex items-center gap-3">
          ...
          <div class="navLinks hidden lg:flex duration-500 absolute md:static md:w-auto w-full md:h-auto h-[85vh] md:items-center gap-[1.5vw] top-[100%] px-5 md:py-0 py-5">
            <ul class="flex md:flex-row flex-col md:items-center md:gap-[2vw] gap-8">
              <RouterLink
                to="/all"
    ...
    <div v-if="store.userData.type == 2">
      <nav v-if="!route.meta?.hideNavbar" class="flex bg-base-black relative shadow-lg px-3 py-2 justify-between flex-row-reverse">
        <div class="hidden lg:flex items-center">
          <div class="flex gap-5 justify-between items-center">
            <div class="relative">
              ...
              <input type="search" placeholder="Search for vehicles by make, model, VIN, etc." class="border-[1px] rounded-[8px] w-full bg-transparent border-[#d3d3d361] py-3 px-10" />
            </div>
            ...
            <div class="navbar-right relative">
              <button class="relative z-50 block overflow-hidden leading-loose rounded cursor-pointer" @click="isOpen = !isOpen">
                <div class="w-8 h-8">
                  <img v-if="store?.userData?.dealer?.picture" class="w-full shadow-md rounded-full h-full object-cover" :src="bucket + store?.userData?.dealer?.picture" alt="" />
                  <img
                    v-else
                    class="w-full shadow-md rounded-full h-full object-cover"
                    src="https://media.istockphoto.com/id/1016744004/vector/profile-placeholder-image-gray-silhouette-no-photo.jpg?s=612x612&w=0&k=20&c=mB6A9idhtEtsFXphs1WVwW_iPBt37S2kJp6VpPhFeoA="
        ...
        <div class="flex items-center gap-3">
          ...
          <div class="navLinks duration-500 absolute md:static md:w-auto w-full md:h-auto h-[85vh] hidden lg:flex md:items-center gap-[1.5vw] top-[100%] px-5 md:py-0 py-5">
            <ul class="flex md:flex-row flex-col md:items-center md:gap-[2vw] gap-8">
              <RouterLink
                to="/upcoming"
    ...
    <template v-if="store?.userData?._id">
      <div v-show="open" class="fixed bg-[#0b1107b5] left-0 animation-menu-modal top-0 h-screen w-full z-[580]"></div>
      <nav v-show="open" class="fixed animation-menu-modal top-0 lg:hidden z-[1000] bg-[#141f0d] w-full p-5">
        <div class="flex gap-3 justify-between items-center">
          ...
          <svg @click="open = false" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
            <path
        ...
        <div v-if="store.userData.type == 1" class="flex flex-col gap-4 mt-4">
          <ul class="flex flex-col gap-3">
            ...
            <RouterLink
              to="/liveSeller"
              :class="path == 'liveSeller' || path == 'upcoming-auctions-seller' || path == 'completed-seller' || path == 'current-bits-seller' ? ' text-primary' : ' text-white'"
              class="relative max-w-fit rounded-[8px] hover:text-primary ease-linear duration-500"
            >
              <p>Bid on a Car</p>
            </RouterLink>
          ...
          <div class="px-2 flex flex-col pt-4 gap-5 border-t border-[#333333]">
            <div @click="goAccount" class="flex items-center gap-2" v-if="store?.userData?.type == 1">
              <img v-if="store?.userData?.seller?.picture" class="h-10 rounded-full w-10 object-cover" :src="bucket + store?.userData?.seller?.picture" alt="" />
              <img
                v-else
                ...
                alt=""
              />
              <p class="text-white text-sm font-semibold truncate w-[150px]">{{ store?.userData?.seller?.firstName }} {{ store?.userData?.seller?.lastName }}</p>
            </div>
            <div @click="goAccount" v-else class="flex items-center gap-2">
              <img v-if="store?.userData?.dealer?.picture" class="h-10 rounded-full w-10 object-cover" :src="bucket + store?.userData?.dealer?.picture" alt="" />
              <img
                v-else
                class="h-10 rounded-full w-10 object-cover"
                src="https://media.istockphoto.com/id/1016744004/vector/profile-placeholder-image-gray-silhouette-no-photo.jpg?s=612x612&w=0&k=20&c=mB6A9idhtEtsFXphs1WVwW_iPBt37S2kJp6VpPhFeoA="
        ...
        <div v-else class="flex flex-col gap-4 mt-4">
          ...
          <div class="px-2 flex flex-col pt-4 gap-5 border-t border-[#333333]">
            <div @click="goAccount" class="flex items-center gap-2" v-if="store?.userData?.type == 1">
              <img v-if="bucket + store?.userData?.seller?.picture" class="h-10 rounded-full w-10 object-cover" :src="bucket + store?.userData?.seller?.picture" alt="" />
              <img
                v-else
                class="h-10 rounded-full w-10 object-cover"
                src="https://media.istockphoto.com/id/1016744004/vector/profile-placeholder-image-gray-silhouette-no-photo.jpg?s=612x612&w=0&k=20&c=mB6A9idhtEtsFXphs1WVwW_iPBt37S2kJp6VpPhFeoA="
            ...
            <div @click="goAccount" v-else class="flex items-center gap-2">
              ...
              <img
                ...
                alt=""
              />
              <p class="text-white text-sm font-semibold truncate w-[150px]">{{ store.userData?.dealer?.firstName }} {{ store.userData?.dealer?.lastName }}</p>
            </div>
            <div @click="logout" class="flex gap-2">
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                <path
    ...
    <div
      :class="[
        scrollPosition > 6 ? 'top-0' : ' top-[2%] md:top-[56px]',
        show ? 'visible opacity-100 transition-all ease-out duration-500 ' : ' invisible opacity-0  transition-all ease-out duration-300 ',
      ]"
    ...
    >
      <div
        v-for="(item, index) in notiAutions"
        :key="index"
      ...
      >
        <RouterLink :to="{ name: 'action-details-dealer', params: { id: item._id } }" class="grid grid-cols-6 place-content-start place-items-start gap-3">
          <div class="col-span-2 w-[90%] md:h-[100px]">
            <img class="rounded-lg shadow-lg w-full h-full object-cover" :src="bucket + item.vehicleDetails.exteriorPhotos[0]" alt="" />
          </div>
          <div class="col-span-4">
            <div class="flex items-start gap-2">
              <svg v-if="item.bidExceeded" class="h-[17px] w-[17px] md:h-[25px] md:w-[25px]" xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 16 17" fill="none">
                <path
              ...
              <svg class="md:w-[25px] w-4 h-4 md:h-[25px]" v-if="item.bidsFinished" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 17" fill="none">
                <path
              ...
              <p v-if="item.bidsFinished" class="text-[#05A54B] text-xs md:text-lg">Congratulations, You've Won!</p>
            </div>
            <p v-if="item.bidExceeded" class="text-xs md:text-lg">
              Your bid for the
              <span class="font-bold">{{ item.vehicleDetails?.year }} {{ item.vehicleDetails?.make }}{{ item.vehicleDetails?.model }}</span>
              has been outbid, keep bidding to win!
            </p>
            <p v-if="item.bidsFinished" class="text-xs md:text-lg">
