<template>
    <nav class="bg-[#22282F] flex flex-nowrap overflow-auto scroll-custom-options items-center gap-6 py-4 px-3 md:p-5 ">
        ...
        <RouterLink to="/all" :class="path == 'all' ? 'bg-primary' : 'bg-[#464B50] text-white'"
            ...
            <p v-if="loading" class=" whitespace-nowrap text-xs md:text-base ">Loading... (
            <div :class="path == 'all' ? 'text-[#464B50]' : 'text-primary'" class="lds-dual-ring"></div>)
            </p>
            <p v-else class=" whitespace-nowrap text-xs md:text-base ">All ({{ all?.length }})</p>
        </RouterLink>
        <RouterLink to="/upcoming-auctions"
            ...
            <p v-if="loading" class=" whitespace-nowrap text-xs md:text-base ">Loading... (
            <div :class="path == 'upcoming-auctions' ? 'text-[#464B50]' : 'text-primary'" class="lds-dual-ring"></div>)
            </p>
            <p v-else class=" whitespace-nowrap text-xs md:text-base ">Upcoming Auctions ({{ upcoming?.length }})
            </p>
        </RouterLink>
        <RouterLink to="/live" :class="path == 'live' ? 'bg-primary' : 'bg-[#464B50] text-white'"
            ...
            <p v-if="loading" class=" whitespace-nowrap text-xs md:text-base ">Loading... (
            <div :class="path == 'live' ? 'text-[#464B50]' : 'text-primary'" class="lds-dual-ring"></div>)</p>
        ...
        <RouterLink to="/completed" :class="path == 'completed' ? 'bg-primary' : 'bg-[#464B50] text-white'"
            ...
            <p v-if="loading" class=" whitespace-nowrap text-xs md:text-base ">Loading... (
            <div :class="path == 'completed' ? 'text-[#464B50]' : 'text-primary'" class="lds-dual-ring"></div>)</p>
            <p v-else class=" whitespace-nowrap text-xs md:text-base ">Completed ({{ completed?.length }})</p>
        </RouterLink>
    </nav>
</template>
        ...
        watch(autionUpdate, async (newQuestion, oldQuestion) => {
            if (newQuestion) {
                if (newQuestion.status == "live") {
                    ...
                    let index = live.value.findIndex((item) => item._id === newQuestion._id);
                    if (index === -1) {
