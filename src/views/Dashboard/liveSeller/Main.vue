<template>
  ...
  <template v-else>
        ...
        <div class="w-full lg:w-[76%]">
          <div class="flex items-center px-3 justify-between mb-4">
            <p class="text-xs font-semibold md:text-base">
              {{ sortedData?.length }} Vehicles
            </p>
            <div class="flex items-center gap-2">
              <p class="mt-2 hidden md:block">Sort by:</p>
              <div class="flex items-center gap-5">
                <div class="navbar-right relative">
                  <button @click="isOpen = !isOpen" class="flex gap-2 rounded-md shadow-md px-2 bg-white items-center">
          ...
          <div class="p-2" v-if="sortedData.length > 0" :class="changeLayouts
            ? 'grid md:grid-cols-2 lg:grid-cols-3 place-content-center place-items-center gap-5'
            : 'animate-fade-up  animate-ease-in-out animate-delay-200'
            ">
            <div v-for="(auction, index) in sortedData" :key="index"
              class="bg-white flex md:mb-7 gap-5 items-start shadow-steps mb-[20%] w-full" :class="changeLayouts
                ? 'animate-fade-up  animate-ease-in-out animate-delay-200'
                : ''
                ">
    ...
    <ModalBidNow v-if="statusModalBids.isActive" :form="formData" />
  </template>
</template>
  ...
  setup() {
    ...
    watch(autionUpdate, async (newQuestion, oldQuestion) => {
      if (autionUpdate.value.status == "live") {
        ...
        let newArray = data.value.map((aution) =>
          aution._id !== newQuestion._id ? aution : newQuestion
        );
        data.value = [];
        data.value = newArray;
      }
      if (autionUpdate.value?.status == "completed") {
        ...
        result = data.value.filter(
          (remove) => remove._id !== autionUpdate.value._id
        );
        data.value = [];
        data.value = result;
      }
      ...
      try {
        ...
        if (res) {
          ...
          data.value.map((autions, index) => {
            ...
            autions.vehicleDetails.odometer = formatter?.format(
              autions.vehicleDetails.odometer
            );
            let photos = null;
    ...
    function timeToEnd(startDate, duration) {
      ...
      return (
        new Date(
          new Date(startDate).getTime() + duration * 1000 * 60
        ).valueOf() - Date.now()
      );
    }
      ...
      switch (sortBy.value) {
          ...
          for (let key of statusOrder) {
            if (sortStatus[key])
              sortedItems.push(
                sortStatus[key].sort(
                  (a, b) =>
                    timeToEnd(a.startDate, a.duration) -
                    timeToEnd(b.startDate, b.duration)
                )
              );
          }
        ...
        case "Year":
          return data.value.sort(
            (a, b) =>
              parseFloat(b.vehicleDetails.year) -
              parseFloat(a.vehicleDetails.year)
          );
        default:
          return data.value;
      }
    ...
    watch(statusModalBids, async (newQuestion, oldQuestion) => {
