    ...
    <template v-else>
        <div class="text-blue-800  md:grid hidden h-custom-chat overflow-hidden relative  grid-cols-9 grid-rows-7 ">
                ...
                <div class="flex items-center justify-between gap-4">
                    <div class="flex justify-between w-full items-center">
                        <div class="flex gap-2 items-center">
                            <img v-if="selectedChat?.auction"
                                class="w-[60px] rounded-full shadow-lg h-[60px] object-cover"
                                :src="bucket + selectedChat?.auction?.vehicleDetails?.exteriorPhotos[0]" alt="">
                            ...
                            <div class="flex items-start  flex-col">
                                ...
                                <p class="capitalize font-medium">{{ selectedChat?.auction?.vehicleDetails?.year }}
                                    {{
                                        selectedChat?.auction?.vehicleDetails?.make }} {{
                                        selectedChat?.auction?.vehicleDetails?.model }}</p>
                            </div>
                        </div>
                        <!--   <div v-if="auth?.userData.type == 2">
                            <div class="flex gap-2 items-center">
                                ...
                                <div class="flex items-start  flex-col">
                                    <p class="capitalize text-2xl font-medium">
                                        {{ storeAution?.owner?.seller.firstName }} {{
                                            storeAution?.owner?.seller.lastName }}
                                    </p>
                                    <p class="capitalize font-medium">{{ storeAution?.vehicleDetails?.year }} {{
                                        storeAution?.vehicleDetails?.make }} {{ storeAution?.vehicleDetails?.model }}
                                    </p>
                                </div>
                            </div>
                        </div>
                        <div v-else>
                            <div v-if="selectedChat?.participant?.type == 1" class="flex gap-2 items-center">
                                <div class="flex gap-2 items-center">
                                    <img v-if="storeAution?.owner?.seller.picture"
                                        class="w-[60px] rounded-full shadow-lg h-[60px] object-cover"
                                        :src="bucket + storeAution?.owner?.seller.picture" alt="">
                            ...
                            <div v-if="selectedChat?.participant?.type == 2" class="flex gap-2 items-center">
                                <img v-if="selectedChat?.auction"
                                    class="w-[60px] rounded-full shadow-lg h-[60px] object-cover"
                                    :src="bucket + selectedChat?.auction?.vehicleDetails?.exteriorPhotos[0]" alt="">
                                <img v-else class="w-[60px] rounded-full shadow-lg h-[60px] object-cover "
                                ...
                                <div class="flex items-start  flex-col">
                                    <p class="capitalize text-2xl font-medium">
                                        {{ selectedChat?.participant?.dealer.firstName }} {{
                                            selectedChat?.participant?.dealer.lastName }}
                                    </p>
                                    <p class="capitalize font-medium">{{ selectedChat?.auction?.vehicleDetails?.year }}
                                        {{
                                            selectedChat?.auction?.vehicleDetails?.make }} {{
                        ...
                        <RouterLink
                            ...
                            <template v-if="auth?.userData.type == 1">
                                ...
                                <div class="w-full">
                                    ...
                                    <p class="capitalize">{{ user?.andrees }}</p>
                                </div>
                            </template>
                            <template v-else>
                                <div class="w-[30%] ">
                        ...
                        <template v-if="dataChat?.length > 0" v-for="(msg, index) in dataChat" :key="index">
                            ...
                            <div v-if="msg.me" class="flex justify-end pt-4 mb-4" :class="index == 0 ? 'pt-5' : ''">
                                <div>
                                    ...
                                    <div class=" bg-[#fff]  flex items-start flex-col  "
                                        v-else-if="msg.typeMedia == 'pdf'">
            ...
            <div :class="activateLayout ? 'visible pointer-events-auto' : 'invisible pointer-events-none '"
                ...
                <template v-if="auth?.userData?._id === storeAution?.owner?._id">
                    ...
                    <div v-if="storeAution?.status == 'bids completed'">
                        <div class="flex items-center gap-3 mb-3 justify-between  ">
                            <button class="btn w-full bg-primary " @click="openModal(storeAution)">Accept</button>
                ...
                <template v-else>
                        ...
                        <div class="flex flex-col w-full h-full gap-2 border border-[#E0E0E0] p-4 rounded-lg pb-2 ">
                            ...
                            <div class="flex gap-2 items-center">
                                ...
                                <p v-if="storeAution?.status == 'live'" class=" capitalize font-semibold text-xl">{{
                                    storeAution?.status }} </p>
                                <p v-if="storeAution?.status == 'bids completed'"
                    ...
                    <div class="p-2 flex  gap-2 flex-col ">
                        ...
                        <template v-if="storeAution?.status == 'bids completed'">
                            <CurrencyInput :options="{ currency: 'USD' }" v-model="currencyBit"
                            ...
                            <p class="text-[#858585] ">Please enter your revised final bid amount that has been agreed
                                upon
                                with
                                the
                                seller</p>
                    ...
                    <div v-if="storeAution?.status == 'completed'">
                        <RouterLink
                            :to="{ name: auth?.userData.type == 1 ? 'action-details-seller' : 'action-details-dealer', params: { id: storeAution?._id } }"
                ...
                <template v-else>
                    <template v-if="dataTableSearch.length > 0" v-for="(user, indexx) in dataTableSearch" :key="indexx">
                        <RouterLink
                            ...
                            <template v-if="auth?.userData.type == 1">
                                <template v-if="user.participant?.type == 2">
                                    <div class="w-[30%] ">
                                ...
                                <template v-if="user.participant?.type == 1">
                                    <div class="w-[30%] ">
                                        <img v-if="user.participant?.seller?.picture"
                                            :src="bucket + user.participant?.seller?.picture"
                                            class="object-cover h-12 w-full rounded-md" alt="" />
                                        <img v-else class="w-full rounded-s-lg h-12 object-cover"
                                            src="@/assets/img/jpg/image.jpg" alt="">
                                    </div>
                                    <div class="w-full">
                                        <div class="text-lg font-semibold">{{ user.participant?.seller?.name }}</div>
                                        <span class="text-gray-500">{{ user.createdAt }} </span>
                                    </div>
                                </template>
                            </template>
                                    ...
                                    <div>
                                        <p class="text-sm font-semibold">
                                            {{ user.auction?.vehicleDetails?.year }} {{
                                                user.auction?.vehicleDetails?.make
                                            }}
        ...
        <div v-show="showModeMobil" class="md:hidden relative bg-white ">
            ...
            <div class="w-full bg-white  pt-2 fixed bottom-0">
                <div class="flex whitespace-pre gap-2 pb-3 px-2 items-center overflow-x-auto overflow-y-hidden">
                    <p class="text-[9px] p-1 border border-[#0A0A0A] rounded-xl ">Are there any customization or
                        aftermarket
                        modifications on the vehicle?</p>
                    <p class="text-[9px] p-1 border border-[#0A0A0A] rounded-xl ">Do you have service records available
                        for
                        the
                        vehicle?</p>
                    <p class="text-[9px] p-1 border border-[#0A0A0A] rounded-xl ">Are there any known mechanical issues
                        or
                        repairs needed?</p>
                    <p class="text-[9px] p-1 border border-[#0A0A0A] rounded-xl ">Is the vehicle still under warranty,
                        and
                        if
                        so, for how long?</p>
                ...
                <div :class="termins && listChat.length === 0 ? 'bg-[#A3A3A3] pointer-events-none ' : ''"
                    ...
                    <input v-model="textForm" v-on:keyup.enter="sendMessage"
                        class="w-full input-shat bg-transparent   rounded-xl focus:outline-none  focus:ring-0 "
                        type="text" placeholder="type your message here..." />
                    <div class="flex justify-between items-center w-full">
                        <div class="flex gap-2">
...
export default {
    ...
    setup() {
        ...
        const getChats = async (id) => {
            auth.socketChat?.emit("getChats", (response) => {

                console.log('response ------------------- getChats', response)
                let name = null;
                let andrees = null;
                response.map((chats, index) => {
                    ...
                    if (auth?.userData.type == 2) {
                        ...
                        if (chats.auction?.city && chats.auction?.province) {
                            andrees = `${chats.auction?.city}, ${chats.auction?.province}`
                        } else {
                            andrees = ''
                        }
                        if (auth?.userData._id !== chats.auction?.owner?._id) {
                            name = `${chats.auction?.owner?.seller?.firstName} ${chats.auction?.owner?.seller?.lastName} `
                        } else {
                            if (chats.auction?.vehicleDetails.year && chats.auction?.vehicleDetails.make && chats.auction?.vehicleDetails.model) {
                                name = `${chats.auction?.vehicleDetails.year} ${chats.auction?.vehicleDetails.make} ${chats.auction?.vehicleDetails.model}`
                            } else {
                                if (chats.participant.type == 2) {
                                    name = `${chats.participant?.dealer.firstName} ${chats.participant?.dealer.name}`
                                } else if (chats.participant.type == 1) {
                                    name = `${chats.participant?.seller.firstName} ${chats.participant?.seller.lastName}`
                                }
                            }
                        }
        ...
        const sendMessage = async (params) => {
            ...
            try {
                ...
                auth.socketChat?.emit("createMessage", dataSend, (data) => {
        ...
        const getMessages = async (userF, index, id, screen) => {
            ...
            auth.socketChat?.emit("getMessages", dataGet, (response) => {
                console.log('getMessages response >>>>>>>>>>>>>>>', response)
                let name = null;
                if (auth?.userData.type == 2) {
                    name = `${response.auction?.owner?.seller?.firstName} ${response.auction?.owner?.seller?.lastName} `

                } else {
                    if (response.auction?.vehicleDetails.year && response.auction?.vehicleDetails.make && response.auction?.vehicleDetails.model) {
                        name = `${response.auction?.vehicleDetails.year} ${response.auction?.vehicleDetails.make} ${response.auction?.vehicleDetails.model}`
                    } else {
                        if (response.participant.type == 2) {
                            name = `${response.participant?.dealer.firstName} ${response.participant?.dealer.name}`
                        } else if (response.participant.type == 1) {
                            name = `${response.participant?.seller.firstName} ${response.participant?.seller.lastName}`
                        }
                    }
                }
        ...
        const readMessages = (Messages = false) => {
            ...
            auth.socketChat?.emit("setMessagesRead", dataGet, (response) => {
                setTimeout(() => {
                    let countMessages =
        ...
        const openModal = () => {
            console.log('maldito', storeAution.value)
            if (auth.userData.address) {
                console.log('entro en el if address')
                if (auth.userData.seller.phoneValidated) {
                    console.log('entro en el if validate')
                    statusModal.openModal({ isActive: true, data: storeAution.value })
                } else {
        ...
        const acceptAution = async (url) => {
            loading.value = true
            try {
                let res = storeIdAution.acceptAutions(statusModal.dataAutiont._id, url)
                if (res) {
                    return '200'
                }
            } catch (error) {
                ...
                loadingMain.value = true; // Comenzamos con loading activado

                try {
                    const res = await auth.authProfile();
                    auth.initSockets();
                    if (res.data) {
                        await getChats();
                        if (route.query.id) {
                            let idAution = route.query.id.split('-');
                            await getDataAution(idAution[0]);
                            await getMessages();
                        } else {
                            activateLayout.value = false;
                        }
                        if (auth.socketChat) {
                            auth.socketChat.on("newMessageSended", () => {
                                getMessages();
                            });

                            auth.socketChat.on("newMessageReceived", () => {
                                getMessages();
                            });
                        } else {
                            console.error("Socket de chat no inicializado correctamente");
                        }
                    }
                } catch (error) {
                    console.log('Error en onMounted:', error);
